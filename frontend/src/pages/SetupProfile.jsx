...
const handleSubmit = async () => { // 1. เพิ่ม async ตรงนี้
    try {
        ...
        const currentUserId = session?.id || backupUserId;
        console.log("Current ID being used:", currentUserId);
        console.log("Current Session:", session);
        if (!currentUserId) {
            alert("ไม่พบข้อมูล Session กรุณาล็อกอินใหม่อีกครั้งครับโก๋!");
            navigate('/login');
            return;
        }
        // ✅ ยิง Axios ไปที่ Path profile ของโก๋
        await axios.patch(`${import.meta.env.VITE_API_URL}/auth/profile/${currentUserId}`, userPayload);
